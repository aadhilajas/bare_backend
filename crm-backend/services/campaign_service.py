...
If the channel stub is unreachable the campaign still moves to "sent" — messages
stay in "queued" state and will be picked up when the stub comes online (or the
demo can re-send). This keeps the UI demo-able without requiring the stub.
...
from config import CHANNEL_STUB_URL
...
async def expand_and_dispatch(campaign_id: str) -> None:
    ...
    with Session(engine) as session:
        ...
        # Dispatch each message to the channel stub
        async with httpx.AsyncClient(timeout=10.0) as client:
            for msg in messages:
                ...
                try:
                    await client.post(f"{CHANNEL_STUB_URL}/send", json=payload)
                except Exception as exc:
                    # Stub unreachable: log but do not fail the campaign
                    print(f"[warn] channel stub unreachable for message {msg.id}: {exc}")
